...
6. 两边完成后，按 findings first 的方式综合结果，去重并合并 reviewer 间的重复发现。重叠发现权重更高；分歧由你自行判断；summary 保持简短。
7. 如果 review 后用户要求修复，由你负责实现和验证。
8. 如果需要继续 review（通常是上一轮 reviewers 提出的内容被修复，或进行了其它会变更 diff 的操作），则发送内容以告知上一轮的 reviewers 根据最新情况进行审查（如不具备这种发送能力则视最新情况决定是否有必要发起新一轮的审查）
