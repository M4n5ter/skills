...
最重要的是，这个 skill 要推动 reviewer 对代码结构保持野心，但野心以**净减少复杂度**衡量，不以重写范围衡量。主动寻找代码柔道式的重组：在不改变行为的前提下，让实现显著更简单、更小、更直接；不能具体说明会删除哪些概念、分支或模块依赖的重组，不得作为阻塞项。
...
- 是否存在低价值测试或垃圾测试，例如复述实现、mock 内部协作者、断言调用次数/顺序/低智文本断言、测试 private method，或用脆弱 setup 伪造信心（包括但不局限于这些示例）
...
> 保持野心。如果存在一条范围受控、能明确减少概念或状态的清晰路径，就推动它；不要用未经证明的大范围重构扩大当前 PR。
> 极其彻底、严谨。三思而后行。
...
0. **对结构性简化保持野心。**
   ...
   - 主动检查是否存在一个代码柔道动作：更有效地借用既有架构，让改动显著更简单；无法证明净收益时，不要假设它存在。
   - 如果看到删除复杂度而不是重新摆放复杂度的路径，要强力推动。
   - 把重组列为阻塞项前，必须说明它会净删除哪些概念、分支或模块依赖，并证明修复范围不会反过来让 PR 膨胀；否则只作为非阻塞探索方向。

1. **把 1000 行视为强制结构审视的警戒线，而不是按数字裁决的死亡线。**
   - diff 让文件跨过该线时，必须显式检查职责数量、可分离所有权、读者工作集、分支增长和未来扩张方向。
   - 只有存在可信的提取边界，能让一个模块独立拥有内聚职责，并实质减少概念、分支或跨层泄漏时，体量增长才构成阻塞项。发现中必须说清该边界及其收益。
   - 不要仅凭行数要求拆分。若文件是一套内聚状态机、解析器、协议表或真实集成场景，拆开后反而增加跨文件编排和同时阅读成本，可以保留大文件。
   - 优先遵循项目语言和社区惯例。测试与实现同文件的项目（例如常见 Rust 布局）尤其应先判断职责和测试价值，不要为压行数机械搬运测试。
   - 无法说明拆分如何降低认知负担时，不要把跨线本身报告为问题；改为审查文件内部导航、命名与局部结构是否足够清晰。
...
9. **让每份复杂度由真实场景买单。**
   - 把每个新增概念、分支、恢复行为和测试追溯到受支持场景、明确契约或必须维护的系统不变量。
   - 不要为了违反明确契约的调用或无法通过系统入口产生的组合扩张实现。若该问题揭示边界设计不佳，优先让非法状态不可表达。
   - 连续审查若总在同一设计边界发现同类问题，或每次修复都继续增加概念、分支和成组测试，把它视为基础模型可能有误的信号，停止局部补丁并重新表述问题。
   - 审查结束前做减法审计：删除没有真实场景的防御逻辑，合并重复表达同一行为的测试，并区分“更完整”与“更正确、更简单”。
...
- 每个新增状态轴和边界分支由哪个真实场景或明确不变量支付？
- 这份复杂度来自目标本身，还是来自连续 review 后逐层堆叠的防御？
...
- PR 导致某个文件跨过 1000 行，并且存在能降低认知负担的清晰职责边界，却仍把它们混在一起。
...
- 这个改动把文件推过了 1000 行，而且持久化与展示投影已有独立所有权。能否沿这个边界拆分，减少读者必须同时跟踪的状态？
...
- 跨过体量警戒线的文件仍然职责内聚，或已沿能降低认知负担的真实边界拆分
...
- 每个新增状态轴、恢复路径和测试族都能追溯到真实场景或明确系统不变量
- 没有因为连续 review 而保留可由更干净契约消除的防御性复杂度

把以下情况视为默认阻塞项：

- PR 保留了大量偶然复杂度，而存在可信的代码柔道动作可以删除这些复杂度
- PR 把文件推过 1000 行，同时混入可独立拥有的职责；存在可信拆分可以减少概念或分支，而不是只重新摆放代码
